...
export const query_insert = (body: object, tabla: string) => {
  ...
  const values_body = Object.values(body).map((value) =>
  ...
  for (let i = 0; i < llaves_body.length; i++) {
    if (llaves_body[i] !== "token_acceso" && values_body[i] !== "") {
      query += ` "${llaves_body[i].replaceAll("'", "")}"${
        i !== llaves_body.length - 1 ? "," : " )"
      } `;
      values +=
        llaves_body[i] === "password"
          ? ` PGP_SYM_ENCRYPT('${values_body[i]}', 'AES_KEY')${
              i !== llaves_body.length - 1 ? "," : " )"
            } `
          : ` '${values_body[i]}'${i !== llaves_body.length - 1 ? "," : " )"} `;
    }
  }
  ...
  bodys.forEach((body: object, index) => {
    ...
    for (let i = 0; i < llaves_body.length; i++) {
      if (llaves_body[i] !== "token_acceso" && values_body[i] !== "") {
        query +=
          index === 0
            ? `${llaves_body[i]}${i !== llaves_body.length - 1 ? "," : " ) "}`
            : "";
        values +=
          llaves_body[i] === "password"
            ? ` PGP_SYM_ENCRYPT('${values_body[i]}', 'AES_KEY')${
...
export const query_update = (body: object, where: object, table: string) => {
  ...
  for (let i = 0; i < llaves_body.length; i++)
    query +=
      values_body[i] !== ""
        ? llaves_body[i] === "password"
          ? `  "${llaves_body[i].replaceAll("'", "")}" = PGP_SYM_ENCRYPT('${
              values_body[i]
            }', 'AES_KEY')${i !== llaves_body.length - 1 ? "," : " )"} `
          : ` "${llaves_body[i].replaceAll("'", "")}" = '${values_body[i]}'${
              i !== llaves_body.length - 1 ? "," : ""
            } `
        : ``;
  for (let i = 0; i < llaves_where.length; i++)
    query_where += ` ${llaves_where[i]} = '${values_where[i]}'${
      i !== llaves_where.length - 1 ? "," : " RETURNING * ;"
    } `;
...
export const query_update_multiple = (
  bodys: Array<object>,
  rows: Array<string>,
  where: object,
  table: string,
  ...
  for (let i = 0; i < rows.length; i++)
    bodys_update = [
      ...bodys_update,
      { name: rows[i], value: bodys.map((item: any) => item[rows[i]]) },
    ];

  for (let i = 0; i < rows.length; i++) {
    query += ` ${bodys_update[i].name} = CASE `;
    for (let j = 0; j < bodys_id.length; j++)
      query += ` WHEN id = ${bodys_id[j]} THEN '${bodys_update[i].value[j]}' `;
    query += `ELSE ${bodys_update[i].name} END ${
      i !== rows.length - 1 ? "," : ""
    } `;
  }
  ...
  for (let i = 0; i < llaves_where.length; i++)
    query += ` ${llaves_where[i]} = '${values_where[i]}'${
      i !== llaves_where.length - 1 ? "," : " RETURNING * ;"
    } `;
